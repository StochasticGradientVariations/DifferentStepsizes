...
def run_adgd(net, n_epoch=2, amplifier=0.02, damping=1, weight_decay=0, eps=1e-8, checkpoint=125, batch_size=128, noisy_train_stat=True):
    ...
    for epoch in range(n_epoch):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            start_time = time.time()  # ΕΝΑΡΞΗ ΜΕΤΡΗΣΗΣ ΧΡΟΝΟΥ minibatch
            ...
            end_time = time.time()  # ΛΗΞΗ ΜΕΤΡΗΣΗΣ ΧΡΟΝΟΥ minibatch
            minibatch_time = end_time - start_time
            print(f"Minibatch {i} took {minibatch_time:.4f} seconds")
...
if __name__ == "__main__":
    ...
    seeds = [int(rng.choice(max_seed, size=1, replace=False)[0]) for _ in range(n_seeds)]
