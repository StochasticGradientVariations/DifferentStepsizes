...
class Adgd(Trainer):
    ...
    """
    def __init__(self, eps=0.0, lr0=None, isVerbose=False, *args, **kwargs):
        if not 0.0 <= eps:
    ...
    def step(self):
        ...
        if self.isVerbose and self.it % 10 == 0:
            print(self.it, la.norm(self.grad), self.loss_func(self.w))
        return self.w - self.lr * self.grad
...
class AdaPGNesterov(Trainer):
    ...
    plus Nesterov‐type momentum.
    """
    def __init__(self, lr0=None, isConservative=False, isVerbose=False, *args, **kwargs):
